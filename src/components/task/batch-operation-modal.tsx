...
export function BatchOperationModal({
  ...
  const handleOperation = async () => {
    ...
    try {
      ...
      toast.success(`Successfully ${getOperationLabel(operation)} ${selectedTasks.length} tasks`);
      onComplete();
      onOpenChange(false);
      setOperation("");
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="grid gap-2">
              ...
              <div
                className={cn(
                  ...
                  operation === "complete" ? "ring-2 ring-primary bg-primary/5" : "hover:shadow"
                )}
                onClick={() => setOperation("complete")}
              >
                <div className="flex items-center gap-3">
                  <div className={cn("p-2 rounded", operation === "complete" && "bg-primary/10")}>
              ...
              <div
                className={cn(
                  "border rounded-lg p-3 cursor-pointer transition-all",
                  operation === "archive" ? "ring-2 ring-primary bg-primary/5" : "hover:shadow"
                )}
                onClick={() => setOperation("archive")}
              >
                <div className="flex items-center gap-3">
                  <div className={cn("p-2 rounded", operation === "archive" && "bg-primary/10")}>
              ...
              <div
                className={cn(
                  ...
                  operation === "delete" ? "ring-2 ring-destructive bg-destructive/5" : "hover:shadow"
                )}
                onClick={() => setOperation("delete")}
              >
                <div className="flex items-center gap-3">
                  <div className={cn("p-2 rounded", operation === "delete" && "bg-destructive/10")}>
              ...
              <div
                className={cn(
                  "border rounded-lg p-3 cursor-pointer transition-all",
                  operation === "move" ? "ring-2 ring-primary bg-primary/5" : "hover:shadow"
                )}
                onClick={() => setOperation("move")}
              >
                <div className="flex items-center gap-3">
                  <div className={cn("p-2 rounded", operation === "move" && "bg-primary/10")}>
              ...
              <div
                className={cn(
                  "border rounded-lg p-3 cursor-pointer transition-all",
                  operation === "set_priority" ? "ring-2 ring-primary bg-primary/5" : "hover:shadow"
                )}
                onClick={() => setOperation("set_priority")}
              >
                <div className="flex items-center gap-3">
                  <div className={cn("p-2 rounded", operation === "set_priority" && "bg-primary/10")}>
              ...
              <div
                className={cn(
                  "border rounded-lg p-3 cursor-pointer transition-all",
                  operation === "add_labels" ? "ring-2 ring-primary bg-primary/5" : "hover:shadow"
                )}
                onClick={() => setOperation("add_labels")}
              >
                <div className="flex items-center gap-3">
                  <div className={cn("p-2 rounded", operation === "add_labels" && "bg-primary/10")}>
          ...
          {operation === "add_labels" && (
            <div className="mt-4">
              ...
              <div className="grid grid-cols-2 gap-2 max-h-60 overflow-y-auto">
                {labels.map(label => (
                  <label key={label.id} className="flex items-center space-x-2 p-2 border rounded cursor-pointer">
                    <Checkbox
                      checked={confirmationInput.split(",").map(n => parseInt(n)).includes(label.id)}
                      onCheckedChange={checked => {
                        if (checked) {
                          setConfirmationInput(prev => {
                            const ids = prev ? prev.split(",").map(n => parseInt(n)) : [];
                            return [...ids, label.id].join(",");
                          });
                        } else {
                          setConfirmationInput(prev => {
                            if (!prev) return "";
                            const ids = prev.split(",").map(n => parseInt(n)).filter(id => id !== label.id);
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              isProcessing ||
              (requireConfirmation && confirmationInput !== getConfirmationInput(operation)) ||
          ...
          >
            {isProcessing ? (
              ...
              <>
                ...
                <span className="ml-2">
                  {operation === "delete" ? "Delete" : "Apply"} {selectedTasks.length} Task{selectedTasks.length !== 1 ? "s" : ""}
                </span>
              </>
            )}
          </Button>
...
async function performBatchAction(operationType: string, confirmationValue: string): Promise<void> {
