...
interface TaskSuccessStoriesProps {
  task?: Task;
  onComplete?: (
    story: Omit<SuccessStory, 'id' | 'created_at' | 'shared'>
  ) => void;
}
...
export function TaskSuccessStories({
  task,
  onComplete,
  ...
  const handleShareStory = (storyId: number) => {
    ...
    if (story) {
      ...
      saveStories(
        savedStories.map(s =>
          s.id === storyId ? { ...s, shared: !s.shared } : s
        )
      );
    }
  };
  ...
  const getSuggestedTags = () => {
    ...
    if (story.key_insight.toLowerCase().includes('time'))
      tags.add('efficiency');
    if (story.key_insight.toLowerCase().includes('team'))
      tags.add('collaboration');
    if (story.key_insight.toLowerCase().includes('problem'))
      tags.add('troubleshooting');
    if (story.key_insight.toLowerCase().includes('improv'))
      tags.add('improvement');
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                onChange={e =>
                  setStory({ ...story, what_went_well: e.target.value })
                }
                rows={3}
              />
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={e =>
                  setStory({ ...story, key_insight: e.target.value })
                }
                rows={2}
              />
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={e =>
                  setStory({ ...story, improvement_suggestion: e.target.value })
                }
                rows={2}
              />
            </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <Badge
                          className={
                            difficultyLabels[story.difficulty_level].color
                          }
                        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-center">
            ...
            <div>
              <p className="text-2xl font-bold text-blue-600">
                {Math.round(
                  (savedStories.filter(s => s.key_insight).length /
                    Math.max(savedStories.length, 1)) *
                    100
                )}
                %
              </p>
            ...
            <div>
              <p className="text-2xl font-bold text-green-600">
                {Math.round(
                  (savedStories.filter(s => s.improvement_suggestion).length /
                    Math.max(savedStories.length, 1)) *
                    100
                )}
                %
