...
const categories = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        keyword: 'priority',
        action: () => ({ type: 'search_tasks' as const, payload: 'priority' }),
      },
    ],
  },
  {
    ...
    items: [
      {
        name: 'New task',
        keyword: 'new',
        action: () => ({ type: 'create_task' as const, payload: '' }),
      },
      {
        name: 'Complete task',
        keyword: 'complete',
        action: () => ({ type: 'complete_task' as const, payload: 0 }),
      },
      {
        name: 'Delete task',
        keyword: 'delete',
        action: () => ({ type: 'delete_task' as const, payload: 0 }),
      },
    ],
  },
  {
    ...
    items: [
      {
        name: 'Today',
        keyword: 'today',
        action: () => ({ type: 'go_to_view' as const, payload: 'today' }),
      },
      {
        name: 'Kanban',
        keyword: 'kanban',
        action: () => ({ type: 'go_to_view' as const, payload: 'kanban' }),
      },
      {
        name: 'Analytics',
        keyword: 'analytics',
        action: () => ({ type: 'go_to_view' as const, payload: 'analytics' }),
      },
      {
        name: 'AI Assistant',
        keyword: 'ai',
        action: () => ({ type: 'open_ai' as const, payload: '' }),
      },
    ],
  },
  {
    ...
    items: [
      {
        name: 'AI parse task',
        keyword: 'ai parse',
        action: () => ({ type: 'open_ai' as const, payload: 'parse' }),
      },
      {
        name: 'Generate project plan',
        keyword: 'project',
        action: () => ({ type: 'open_ai' as const, payload: 'project' }),
      },
      {
        name: 'Decision template',
        keyword: 'decision',
        action: () => ({ type: 'open_ai' as const, payload: 'decision' }),
      },
    ],
  },
  {
    ...
    items: [
      {
        name: 'Focus mode',
        keyword: 'focus',
        action: () => ({ type: 'go_to_view' as const, payload: 'focus' }),
      },
      {
        name: 'Pomodoro timer',
        keyword: 'pomodoro',
        action: () => ({ type: 'go_to_view' as const, payload: 'pomodoro' }),
      },
      {
        name: 'Goals Dashboard',
        keyword: 'goals',
        action: () => ({ type: 'go_to_view' as const, payload: 'goals' }),
      },
    ],
  },
  {
    ...
    items: [
      {
        name: 'Auto-schedule tasks',
        keyword: 'schedule',
        action: () => ({ type: 'go_to_view' as const, payload: 'scheduler' }),
      },
      {
    ...
    items: [
      {
        ...
        keyword: 'team',
        action: () => ({ type: 'go_to_view' as const, payload: 'team' }),
      },
      {
        name: 'Team velocity',
        keyword: 'velocity',
      ...
      },
      {
        ...
        keyword: 'knowledge',
        action: () => ({ type: 'go_to_view' as const, payload: 'knowledge' }),
      },
    ],
  },
  {
  ...
  patterns: [
    {
      ...
      action: (match: string, tasks: TaskWithRelations[]) => {
        ...
        return task
          ? { type: 'complete_task' as const, payload: task.id }
          : null;
      },
    },
    {
      ...
      action: (match: string, tasks: TaskWithRelations[]) => {
        const taskName = match
          .replace(/mark\s+.+\s+as\s+(done|complete)/i, '')
          .trim();
        const task = tasks.find(t =>
          t.name.toLowerCase().includes(taskName.toLowerCase())
        );
    ...
    },
    {
      ...
      action: (match: string, tasks: TaskWithRelations[]) => {
        const parts = match.match(
          /set\s+(priority\s+)?(.+?)\s+to\s+(critical|high|medium|low)/i
        );
    ...
    {
      pattern:
        /schedule\s+(task\s+)?(.+?)\s+(for|on|tomorrow|next\s+(week|monday|tuesday|wednesday|thursday|friday|saturday|sunday))/i,
      action: (match: string, tasks: TaskWithRelations[]) => {
        ...
        const taskName = match
          .replace(/schedule\s+(task\s+)?/, '')
          .split(/\s+(for|on|tomorrow)/i)[0]
          .trim();
        const task = tasks.find(t =>
          t.name.toLowerCase().includes(taskName.toLowerCase())
        );
    ...
    },
    {
      ...
      action: (match: string, _tasks: TaskWithRelations[]) => {
        const taskName = match
          .replace(/(create|add)\s+(a\s+)?(task|todo)\s+/, '')
          .trim();
        return { type: 'create_task' as const, payload: taskName };
      },
    },
    {
      ...
      action: (match: string) => {
        const taskName = match
          .replace(/(remind|remember)\s+me\s+to\s+/, '')
          .trim();
        return { type: 'create_task' as const, payload: taskName };
      },
    },
...
export function CommandPalette({
  ...
  return (
    <CommandDialog open={open} onOpenChange={onOpenChange}>
      <CommandInput
        placeholder="Type a command (e.g., 'complete meeting', 'new task: fix bug', 'go to kanban')"
        ref={inputRef}
      />
        ...
        <CommandGroup heading="NLP Commands">
          <CommandItem
            onSelect={() =>
              executeCommand({
                type: 'create_task',
                payload: 'Schedule team meeting for tomorrow',
              })
            }
          >
          ...
          <CommandItem
            onSelect={() =>
              executeCommand({
                type: 'create_task',
                payload: 'Fix bug in the login form',
              })
            }
          >
