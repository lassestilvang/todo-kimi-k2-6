...
describe('TaskFlowVisualization', () => {
  ...
  it('renders the visualization header', () => {
    render(
      <TaskFlowVisualization tasks={mockTasks} connections={mockConnections} />
    );

    expect(screen.getByText('Task Flow Visualization')).toBeInTheDocument();
    expect(
      screen.getByText(/Visualize task dependencies and progress/)
    ).toBeInTheDocument();
  });
...
describe('TaskFlowVisualization - Dependencies', () => {
  ...
  it('calculates blockers correctly', () => {
    ...
    render(
      <TaskFlowVisualization tasks={incompleteTask} connections={connections} />
    );
