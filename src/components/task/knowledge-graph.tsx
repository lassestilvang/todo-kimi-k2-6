    ...
    <div className="flex items-center justify-center h-full bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-950/20 dark:to-blue-950/20 rounded-lg">
      <div className="text-center max-w-md p-6">
        ...
        <p className="text-muted-foreground mb-4">
          Advanced force-directed graph visualization showing task
          relationships, skill connections, and habit patterns.
        </p>
    ...
    try {
      ...
      tasks.forEach(task => {
        const node: GraphNode = {
          ...
          skill_level: task.completed
            ? Math.min(5, Math.floor(Math.random() * 3) + 1)
            : undefined,
          connection_strength: task.blockers?.length
            ? 1 / Math.max(task.blockers.length, 1)
            : 0.5,
  ...
  const filteredData = () => {
    ...
    if (filterType !== 'all') {
      filteredNodes = nodes.filter(n => {
        ...
        if (filterType === 'in_progress' && n.group === 'in_progress')
          return true;
  ...
  const renderGraph = () => {
    ...
    return (
      <div
        ...
        className="relative w-full h-96 bg-gradient-to-br from-purple-50/50 to-blue-50/50 dark:from-purple-950/20 dark:to-blue-950/20 rounded-lg border border-purple-200/50 dark:border-purple-800/50"
      >
        <FallbackForceGraph
          graphData={filtered}
      ...
      <div className="space-y-4">
          ...
          >
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a semantic relationship between two tasks to show how
        ...
        {connections.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm">
              Create connections to visualize relationships between tasks
            </p>
          </div>
        ) : (
  ...
  const ConnectionForm = ({
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          <Label>
            Connection Strength: {Math.round(formData.strength * 100)}%
          </Label>
          <Slider
            min={0}
            max={1}
            ...
            onValueChange={value =>
              setFormData(prev => ({ ...prev, strength: value[0] }))
            }
          />
        </div>
      ...
      <div className="space-y-4">
        {insights.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm">
              Insights will appear as you complete more tasks
            </p>
          </div>
        ) : (
                ...
                <CardContent className="pt-4">
                  ...
                  <div className="flex flex-wrap gap-1 mt-3">
                    {insight.context_tags &&
                      JSON.parse(insight.context_tags).map((tag: string) => (
      ...
      <div className="space-y-4">
        {skills.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm">
              Complete tasks to build your skills profile
            </p>
          </div>
        ) : (
          <div className="grid gap-4">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div
                        className={`w-3 h-3 rounded-full ${getPriorityColor(selectedNode.priority)}`}
                      />
              ...
              <CardContent>
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm">
                      {
                        tasks.find(t => t.id === selectedLink.source_task_id)
                          ?.name
                      }
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-sm">
                      {
                        tasks.find(t => t.id === selectedLink.target_task_id)
                          ?.name
                      }
                    </p>
                  </div>
                  <div>
                    ...
                    <Badge variant="outline" className="text-xs mt-1">
                      {connectionTypes.find(
                        t => t.value === selectedLink.connection_type
                      )?.label || selectedLink.connection_type}
                    </Badge>
                  </div>
                  <div>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <Badge variant="secondary">
                      {connections.filter(c => c.strength > 0.7).length}
                    </Badge>
                  </div>
                </div>
              </CardContent>
