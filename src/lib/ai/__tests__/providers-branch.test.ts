...
vi.mock("../time-utils", () => ({
  formatMinutesToTime: vi.fn(mins => `${Math.floor(mins / 60)}:${String(mins % 60).padStart(2, "0")}`),
...
describe("AI Providers - Branch Coverage Tests", () => {
  ...
  describe("AICache - All Branches", () => {
    ...
    it("should return cached data when not expired", () => {
      ...
      cache.set("key1", { data: { value: "cached" }, timestamp: Date.now() - 1000 });
    ...
    it("should delete and return null for expired cache", () => {
      ...
      cache.set("key1", { data: { value: "old" }, timestamp: Date.now() - CACHE_TTL_MS - 10000 });
