...
export function TaskComments({
  ...
  const handleSubmit = async () => {
    ...
    const mentionIds = mentionUsernames
      .map(
        username =>
          users.find(u => u.name?.toLowerCase() === username.toLowerCase())?.id
      )
    ...
    setTimeout(() => {
      if (textareaRef.current) {
        textareaRef.current.selectionStart =
          cursorPos + (user.name || user.email).length + 2;
    ...
    <div className="flex flex-col max-h-96">
      ...
      <div className="border-t pt-3">
        ...
        <p className="text-xs text-muted-foreground">
          Press Enter to send, Shift+Enter for new line. Use{' '}
          <AtSign className="h-3 w-3 inline" /> to mention users.
        </p>
      </div>
    </div>
  );
