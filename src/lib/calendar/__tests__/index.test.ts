...
describe('Calendar Index', () => {
  ...
  describe('getCalendarEvents', () => {
    ...
    it('should call outlook getOutlookEvents for outlook provider', async () => {
      ...
      const { getCalendarEvents } = await import('../index');
      const result = await getCalendarEvents(
        { provider: 'outlook', accessToken: 'token' },
        '2026-06-01',
        '2026-06-30'
      );
    ...
    it('should throw error for unknown provider', async () => {
      ...
      await expect(
        getCalendarEvents(
          { provider: 'unknown' as any, accessToken: 'token' },
          '2026-06-01',
          '2026-06-30'
        )
  ...
  describe('exchangeCodeForTokens', () => {
    ...
    it('should throw error for unknown provider', async () => {
      ...
      await expect(
        exchangeCodeForTokens('unknown' as any, 'auth-code')
      ).rejects.toThrow('not yet implemented');
    });
  });
  ...
  describe('syncTasksToCalendar', () => {
    it('should call google syncTasksToCalendar for google provider', async () => {
      mockGoogleSyncTasks.mockResolvedValue({
        created: 1,
        updated: 0,
        errors: [],
      });
