...
describe('Permissions Module', () => {
  ...
  describe('canPerformActionByPermission', () => {
    ...
    it('should allow view action for authenticated user', () => {
      expect(canPerformActionByPermission('user@example.com', 'view')).toBe(
        true
      );
    });
    ...
    it('should allow edit action for editor', () => {
      expect(canPerformActionByPermission('user@example.com', 'edit')).toBe(
        true
      );
    });
    ...
    it('should deny delete action for editor', () => {
      expect(canPerformActionByPermission('user@example.com', 'delete')).toBe(
        false
      );
    });

    it('should allow delete action for admin', () => {
      expect(canPerformActionByPermission('admin@todo.com', 'delete')).toBe(
        true
      );
    });

    it('should deny admin action for editor', () => {
      expect(canPerformActionByPermission('user@example.com', 'admin')).toBe(
        false
      );
    });

    it('should allow admin action for admin', () => {
      expect(canPerformActionByPermission('admin@todo.com', 'admin')).toBe(
        true
      );
    });
    ...
    it('should handle all action types', () => {
      ...
      actions.forEach(action => {
        expect(
          typeof canPerformActionByPermission('user@example.com', action)
        ).toBe('boolean');
      });
    });
  });
