...
import { getTasks, createTask, getTimeReport } from '@/lib/actions';
import {
  syncOfflineTasks,
  getSyncStatus,
  clearSyncedTasks,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const queryParams = {
      view: searchParams.get('view') as
        'today' | 'next7' | 'upcoming' | 'all' | 'blocked' | undefined,
...
export async function GET_TIME_REPORT(request: NextRequest) {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Failed to get time report';
    return errorResponse(message, 500);
  }
}
