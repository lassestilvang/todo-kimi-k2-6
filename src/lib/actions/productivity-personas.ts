...
import { calculateOptimalSchedule, getUserEnergyProfile } from "@/lib/ai/scheduler";

export type PersonaType = "deep_work" | "sprint_runner" | "steady_stream" | "creative_genius" | "strategic_planner";

export type EnergyType = "morning_energy" | "afternoon_focus" | "creative_window" | "recovery_time" | "morning_routine" | "lunch_break" | "end_of_day";
 ...
 */
export async function getUserPersona(userId: number): Promise<PersonaConfig | null> {
  const db = getDb();
...
export async function saveUserPersona(
  ...
  return db.prepare("SELECT * FROM user_personas WHERE user_id = ?").get(userId) as PersonaConfig;
}

/**
 * Analyze user to detect their productivity persona
 */
export async function analyzeUserPersona(userId: number): Promise<PersonaAnalysis> {
  const db = getDb();
  ...
  if (completedTasks.length < 10) {
    return {
      userId,
      currentPersona: await getUserPersona(userId) || getDefaultPersona(userId),
  ...
  if (morningPeaks >= 2) {
    ...
    evidence.completionPatterns = ["Morning peak productivity", "Consistent early output"];
  } else if (eveningPeaks >= 2) {
    detectedType = "creative_genius";
    evidence.completionPatterns = ["Late day creativity", "Evening focus"];
  } else if (peakHours.length > 0 && (peakHours[peakHours.length - 1] - (peakHours[0] || 0)) > 4) {
    detectedType = "sprint_runner";
    evidence.completionPatterns = ["Burst productivity", "Variable energy patterns"];
  }

  evidence.timePreferences = peakHours.map(h => `${h}:00 - ${h + 1}:00`);
  evidence.energyLevels = [`${morningPeaks > afternoonPeaks ? "Morning" : "Afternoon"} preference`];
 ...
 */
export async function calculateTaskDNA(taskId: number, userId: number): Promise<TaskDNAResult> {
  const db = getDb();
  ...
  const complexity = Math.min(1, (dependencyCount * 0.2) + (descriptionLength / 500));
  ...
  const isUrgent = task[0].deadline && new Date(task[0].deadline) < new Date(Date.now() + 3 * 24 * 60 * 60 * 1000);
  const requiredEnergy = Math.min(5, Math.ceil(complexity * 3) + (isUrgent ? 1 : 0));
  ...
  const peakHours = energyProfile.peakEnergyTimes
    .flatMap(range => [parseInt(range.start.split(":")[0]), parseInt(range.end.split(":")[0])]);

  const isInPeak = peakHours.some(h => h === taskHour || (h < taskHour && taskHour < h + 2));
 ...
 */
export async function getPersonaRecommendations(userId: number): Promise<PersonaRecommendation[]> {
  const db = getDb();
  ...
  for (const task of tasks) {
    ...
    if (persona.currentPersona?.type === "deep_work") {
      ...
      if (task.estimate && estimatedMinutes && estimatedMinutes > 120) match += 25;
    ...
    recommendations.push({
      ...
      technique: getPersonaTechnique(persona.currentPersona?.type || "steady_stream"),
      confidence: persona.confidence
    });
  }

  return recommendations
    .sort((a, b) => b.taskMatch - a.taskMatch)
    .slice(0, 5);
}
...
export function getPersonaTypes(): Array<{
  ...
  return [
    ...
    },
    {
      ...
      description: "Plans ahead, works best with long-term vision and structure",
      icon: "📊"
    }
  ];
}