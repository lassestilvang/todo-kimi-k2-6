...
describe("Goals Actions - Comprehensive Tests", () => {
  ...
  describe("getGoals", () => {
    ...
    it("should return all goals", async () => {
      await createGoal({ user_id: 1, name: "Goal 1", target_count: 5, target_unit: "tasks", period: "daily" });
      await createGoal({ user_id: 1, name: "Goal 2", target_count: 10, target_unit: "tasks", period: "weekly" });
  ...
  describe("updateGoalProgress", () => {
    it("should increment progress", async () => {
      const goal = await createGoal({ user_id: 1, name: "Test Goal", target_count: 10, target_unit: "tasks", period: "daily" });
      const updated = await updateGoalProgress(goal.id, 3);
      expect(updated.current_count).toBe(3);
    });

    it("should not exceed target", async () => {
      const goal = await createGoal({ user_id: 1, name: "Test Goal", target_count: 5, target_unit: "tasks", period: "daily" });
  ...
  describe("Goal Milestones", () => {
    it("should create a milestone", async () => {
      const goal = await createGoal({ user_id: 1, name: "Test Goal", target_count: 10, target_unit: "tasks", period: "daily" });
      const milestone = await createGoalMilestone(goal.id, { name: "First milestone", target_count: 5, due_date: "2024-12-31" });
      expect(milestone.name).toBe("First milestone");
      expect(milestone.current_count).toBe(0);
    });

    it("should get milestones for a goal", async () => {
      const goal = await createGoal({ user_id: 1, name: "Test Goal", target_count: 10, target_unit: "tasks", period: "daily" });
    ...
    it("should update milestone progress", async () => {
      ...
      const updated = await updateMilestoneProgress(milestone.id, 3);
      expect(updated.current_count).toBe(3);
    });
