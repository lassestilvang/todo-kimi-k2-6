...
export async function getTimeReport(options?: {
  ...
  const tasks =
    taskIds.length > 0
      ? ((await db
          .prepare(
            `SELECT id, name FROM tasks WHERE id IN (${taskIds.map(() => '?').join(',')})`
          )
          .all(...taskIds)) as { id: number; name: string }[])
      : [];
...
export async function getWeeklyTimeSummary(): Promise<{
  ...
  const entries = db
    .prepare(
      `SELECT task_id, duration_seconds, created_at FROM time_entries WHERE created_at >= ? AND duration_seconds IS NOT NULL`
    )
  ...
  const taskNames = (await db
    .prepare(
      `SELECT id, name FROM tasks WHERE id IN (${sortedTasks.map(([id]) => id).join(',')})`
    )
    .all(...sortedTasks.map(([id]) => Number(id)))) as {
    id: number;
    name: string;
  }[];
