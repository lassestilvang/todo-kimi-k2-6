...
describe('Cache Module - Comprehensive Tests', () => {
  ...
  describe('taskCache.workspace', () => {
    ...
    it('should set and get workspace cache', async () => {
      ...
      expect(await cacheModule.taskCache.workspace.get(1)).toEqual(
        workspaceData
      );
    });
  ...
  describe('taskCache.aiResponses', () => {
    ...
    it('should set and get AI responses', async () => {
      ...
      await cacheModule.taskCache.aiResponses.set(
        "Create a task called 'Test'",
        response
      );

      const key = cacheModule.taskCache.aiResponses.key(
        "Create a task called 'Test'"
      );
      expect(
        await cacheModule.taskCache.aiResponses.get(
          "Create a task called 'Test'"
        )
      ).toEqual(response);
    });
    ...
    it('should return null for non-existent AI response', async () => {
      expect(
        await cacheModule.taskCache.aiResponses.get('nonexistent prompt')
      ).toBeNull();
    });
  ...
  describe('taskCache.integrations', () => {
    ...
    it('should set and get user integrations', async () => {
      ...
      expect(await cacheModule.taskCache.integrations.get(1)).toEqual(
        integrations
      );
    });
