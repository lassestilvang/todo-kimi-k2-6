...
test.describe("Task Modal Component", () => {
  ...
  test("should open task modal when clicking new task button", async ({ page }) => {
  ...
  test("should create task with all fields", async ({ page }) => {
    ...
    // Fill basic info
    await page.fill('input[placeholder="What needs to be done?"]', "Complete Project Report");

    // Fill description
    await page.fill('textarea[placeholder="Add details..."]', "Detailed report on Q3 performance");
  ...
  test("should cancel modal without saving", async ({ page }) => {
    ...
    await page.fill('input[placeholder="What needs to be done?"]', "This should not be saved");
  ...
  test("should edit existing task", async ({ page }) => {
    ...
    // Edit the task name
    await page.fill('input[placeholder="What needs to be done?"]', "Task to Edit Updated");
  ...
  test("should add labels to task", async ({ page }) => {
    await page.click('button:has-text("New Task")');
    await page.fill('input[placeholder="What needs to be done?"]', "Task with Labels");
    await page.click('button:has-text("Create Task")');
