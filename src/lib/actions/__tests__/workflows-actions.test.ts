...
describe('Workflow Actions', () => {
  ...
  describe('updateWorkflow', () => {
    it('should update workflow name', async () => {
      ...
      const updated = await updateWorkflow(1, created.id, {
        name: 'Updated Name',
      });
    ...
    it('should throw error for non-existent workflow', async () => {
      ...
      await expect(
        updateWorkflow(1, 99999, { name: 'Updated' })
  ...
  describe('deleteWorkflow', () => {
    it('should delete a workflow', async () => {
      const { createWorkflow, deleteWorkflow, getWorkflow } =
  ...
  describe('toggleWorkflow', () => {
    ...
    it('should throw error for non-existent workflow', async () => {
      ...
      await expect(toggleWorkflow(99999, 1)).rejects.toThrow(
        'Workflow not found'
      );
    });
  });
  ...
  describe('getWorkflowExecutions', () => {
    ...
    it('should filter executions by status', async () => {
      const { createWorkflow, getWorkflowExecutions } =
        await import('../workflows');
      ...
      const completed = await getWorkflowExecutions(created.id, {
        status: 'completed',
      });
  ...
  describe('evaluateConditions', () => {
    ...
    it('should parse JSON conditions string', async () => {
      ...
      const result = await evaluateConditions(
        JSON.stringify({ task_priority: 'high' }),
        {
          task_priority: 'high',
        }
      );
  ...
  describe('executeWorkflow', () => {
    ...
    it('should throw error when workflow is disabled', async () => {
      const { createWorkflow, getWorkflow, executeWorkflow } =
        await import('../workflows');
      ...
      db.prepare('UPDATE workflows SET enabled = 0 WHERE id = ?').run(
        workflow.id
      );
      ...
      await expect(executeWorkflow(workflow.id, {}, 1)).rejects.toThrow(
    ...
    it('should throw error when workflow not found', async () => {
      ...
      await expect(executeWorkflow(99999, {}, 1)).rejects.toThrow(
        'Workflow not found or disabled'
      );
    });
