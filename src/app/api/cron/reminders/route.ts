  ...
  try {
    ...
    if (dayOfWeek === 0) {
      ...
      for (const user of users) {
        ...
        const _settings = await shouldSendNotification(
          user.id,
          { id: 0, name: '', description: null, deadline: null },
          'due_soon'
        );
        ...
        const summary = {
          ...
          overdueTasks: userTasks.filter(
            t => t.completed !== 1 && t.deadline && new Date(t.deadline) < now
          ).length,
          criticalTasks: userTasks.filter(
            t => t.priority === 'critical' && t.completed !== 1
          ).length,
        };
