...
describe('Views Actions', () => {
  ...
  describe('updateCustomView', () => {
    it('should throw error for non-existent view', async () => {
      ...
      await expect(
        updateCustomView(999, 1, { name: 'Updated' })
      ).rejects.toThrow('Custom view not found');
    });

    it('should update view name', async () => {
      const { createCustomView, getCustomViews, updateCustomView } =
        await import('../views');
  ...
  describe('deleteCustomView', () => {
    it('should delete a view', async () => {
      const { createCustomView, getCustomViews, deleteCustomView } =
        await import('../views');
