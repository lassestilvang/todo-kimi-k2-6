...
export async function createGoal(
  input: CreateGoalInput & { user_id: number }
): Promise<Goal> {
  const db = getDb();
  const result = db
    .prepare(
  ...
  const goal = db
    ...
    .get(Number(result.lastInsertRowid)) as Goal | undefined;
  return (
    goal || {
...
export async function resetGoal(id: number): Promise<Goal> {
  const db = getDb();
  db.prepare(
    'UPDATE goals SET current_count = 0, streak_count = 0, last_updated = CURRENT_TIMESTAMP WHERE id = ?'
  ).run(id);
  const goal = await getGoalById(id);
  if (!goal) throw new Error('Goal not found');
  return goal;
...
export async function updateGoalsFromTaskCompletion(taskCount = 1) {
  ...
  // Update daily goals
  const dailyGoals = db
    .prepare("SELECT * FROM goals WHERE period = 'daily'")
    .all() as Goal[];
  for (const goal of dailyGoals) {
    const newCount = goal.current_count + taskCount;
    db.prepare(
      'UPDATE goals SET current_count = ?, last_updated = CURRENT_TIMESTAMP WHERE id = ?'
    ).run(Math.min(newCount, goal.target_count), goal.id);
  }
}
...
export async function getGoalMilestones(
  ...
  return db
    .prepare(
      'SELECT * FROM goal_milestones WHERE goal_id = ? ORDER BY due_date ASC, created_at DESC'
    )
...
export async function updateMilestoneProgress(
  ...
  const db = getDb();
  const milestone = db
    .prepare('SELECT * FROM goal_milestones WHERE id = ?')
    .get(id) as GoalMilestone | undefined;
  ...
  db.prepare(
    'UPDATE goal_milestones SET current_count = ?, completed = ? WHERE id = ?'
  ).run(newCount, isCompleted ? 1 : 0, id);

  const updated = db
    .prepare('SELECT * FROM goal_milestones WHERE id = ?')
    .get(id) as GoalMilestone;
  return updated;
}
...
export async function skipGoalMilestone(id: number): Promise<GoalMilestone> {
  ...
  db.prepare(
    'UPDATE goal_milestones SET completed = 1, current_count = target_count WHERE id = ?'
  ).run(id);
