...
> {
  ...
  const [
    ...
    db
      .prepare(
        `SELECT * FROM subtasks WHERE task_id IN (${taskIds.map(() => '?').join(',')}) ORDER BY task_id, id`
      )
      .all(...taskIds) as Subtask[],
    db
      .prepare(
        `SELECT * FROM reminders WHERE task_id IN (${taskIds.map(() => '?').join(',')}) ORDER BY task_id, remind_at`
      )
      .all(...taskIds) as Reminder[],
    db
      .prepare(
        `SELECT * FROM task_logs WHERE task_id IN (${taskIds.map(() => '?').join(',')}) ORDER BY task_id, created_at DESC`
      )
      .all(...taskIds) as TaskLog[],
    db
      .prepare(
        `SELECT * FROM task_comments WHERE task_id IN (${taskIds.map(() => '?').join(',')}) ORDER BY task_id, created_at ASC`
      )
      .all(...taskIds) as TaskComment[],
    db
      .prepare(
        `SELECT td.*, t.name as blocked_task_name FROM task_dependencies td JOIN tasks t ON td.task_id = t.id WHERE td.depends_on_task_id IN (${taskIds.map(() => '?').join(',')})`
      )
      .all(...taskIds) as TaskDependency[],
    db
      .prepare(
        `SELECT td.*, t.name as blocking_task_name FROM task_dependencies td JOIN tasks t ON td.depends_on_task_id = t.id WHERE td.task_id IN (${taskIds.map(() => '?').join(',')})`
      )
      .all(...taskIds) as TaskDependency[],
    db
      .prepare(
    ...
    db
      .prepare(
        `SELECT * FROM task_attachments WHERE task_id IN (${taskIds.map(() => '?').join(',')}) ORDER BY task_id, created_at DESC`
      )
      .all(...taskIds) as TaskAttachment[],
    db
      .prepare(
        `SELECT * FROM time_entries WHERE task_id IN (${taskIds.map(() => '?').join(',')}) ORDER BY task_id, created_at ASC`
      )
      .all(...taskIds) as TimeEntry[],
    db
      .prepare(
        `SELECT * FROM recurring_exceptions WHERE task_id IN (${taskIds.map(() => '?').join(',')}) ORDER BY task_id, exception_date`
      )
      .all(...taskIds) as RecurringException[],
  ]);

  // Group relations by task_id
  const labelsByTask = (labelsResult || []).reduce(
    (acc, label) => {
  ...
  const subtasksByTask = (subtasksResult || []).reduce(
    (acc, subtask) => {
  ...
  const remindersByTask = (remindersResult || []).reduce(
    (acc, reminder) => {
  ...
  const logsByTask = (logsResult || []).reduce(
    (acc, log) => {
  ...
  const commentsByTask = (commentsResult || []).reduce(
    (acc, comment) => {
  ...
  const blockersByTask = (blockersResult || []).reduce(
    (acc, dep) => {
  ...
  const blockedByTask = (blockedByResult || []).reduce(
    (acc, dep) => {
  ...
  const assigneesByTask = (assigneesResult || []).reduce(
    (acc, assignee) => {
  ...
  const attachmentsByTask = (attachmentsResult || []).reduce(
    (acc, att) => {
  ...
  const timeEntriesByTask = (timeEntriesResult || []).reduce(
    (acc, entry) => {
  ...
  const exceptionsByTask = (recurringExceptionsResult || []).reduce(
    (acc, exc) => {
