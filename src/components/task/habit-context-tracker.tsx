...
export function HabitContextTracker({
  taskId,
  context,
  onContextUpdate,
}: HabitContextTrackerProps) {
  const [selectedContextType, setSelectedContextType] =
    useState<HabitContext['context_type']>('time_of_day');
  ...
  const getContextOptions = () => {
    switch (selectedContextType) {
      ...
      case 'mood':
        return moodOptions;
      case 'energy_level':
        return energyLevelOptions;
      default:
        return [];
    }
  };

  const selectedOption = getContextOptions().find(
    o => o.value === selectedContextValue
  );
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-3">
          ...
          <div>
            <Label>
              {selectedContextType === 'energy_level'
                ? 'Energy Level'
                : selectedContextType === 'time_of_day'
                  ? 'Best Time'
                  : selectedContextType === 'location'
                    ? 'Location'
                    : selectedContextType === 'mood'
                      ? 'Mood'
                      : 'Context Value'}
            </Label>
        ...
        {context && (
          <div className="mt-4 p-3 bg-muted/30 rounded-lg">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {context.frequency} recordings • Best at{' '}
              {selectedOption?.label || 'selecting...'}
            </p>
          </div>
        )}
      </CardContent>
