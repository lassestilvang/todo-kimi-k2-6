...
export function TaskFlowVisualization({
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      case "medium": return "bg-amber-500 text-amber-900";
  ...
  const getConnectionTypeLabel = (type: string) => {
    switch (type) {
      ...
      case "learned_from": return "Learned From";
      default: return type;
    }
  };
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                                ...
                                {dependents.length > 0 && (
                                  <span>
                                    Blocks: {dependents.length} tasks
                                  </span>
                                )}
                              </div>
                            </div>
                              ...
                              <div className="space-y-3">
                                <div>
                                  ...
                                  <div className="bg-muted/30 rounded-lg p-3">
                                    <p className="text-xs">{getConnectionTypeLabel("prerequisite")}</p>
                                    <p className="text-xs text-muted-foreground">
                                      Connected to: {task.completed ? "Database Schema" : "API Routes"}
                                    </p>
                                  </div>
                                </div>

                                <div>
                                  <h5 className="text-sm font-medium mb-2">Analytics</h5>
    ...
    <div>
      <div className="grid gap-4">
          ...
          {filteredTasks.map(task => (
            <div key={task.id} className="text-center">
              <Badge variant="outline" className="text-xs">
                {task.name.split(' ').map(w => w[0]).join('').slice(0, 3)}
              </Badge>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {filteredTasks.map(task => (
            <div key={task.id} className="grid grid-cols-[150px_1fr] gap-4 items-center">
              <div className="font-medium text-sm">{task.name}</div>
              <div className="grid gap-2">
                {filteredTasks.map(otherTask => {
                  if (task.id === otherTask.id) return null;
                  const connection = connections.find(
                    c => (c.source_task_id === task.id && c.target_task_id === otherTask.id) ||
                         (c.source_task_id === otherTask.id && c.target_task_id === task.id)
                  );
                  const isBlocker = connection?.connection_type === "prerequisite" && task.id === connection.source_task_id;
                  const isDependent = connection?.connection_type === "prerequisite" && task.id === connection.target_task_id;

                  return (
                    <Badge
                      key={otherTask.id}
                      variant={isBlocker ? "default" : isDependent ? "outline" : "secondary"}
                      className="text-xs cursor-pointer"
                      title={getConnectionTypeLabel(connection?.connection_type || "none")}
                    >
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex gap-2">
                {(["timeline", "matrix", "dependencies"] as ViewMode[]).map(mode => (
                  <Button
                    key={mode}
