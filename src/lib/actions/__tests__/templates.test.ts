...
import { getTemplates, createTemplate, deleteTemplate, saveTemplateFromTask } from "../templates";
...
describe("Templates Actions", () => {
  ...
  describe("deleteTemplate", () => {
    it("should delete a template by id", async () => {
      ...
      // Get the inserted id
      const result = db.prepare("SELECT id FROM templates WHERE name = 'Template to Delete'").get();

      await deleteTemplate(result.id);

      const template = db.prepare("SELECT * FROM templates WHERE id = ?").get(result.id);
  ...
  describe("saveTemplateFromTask", () => {
    it("should save a task as a template", async () => {
      ...
      `);
      const task = db.prepare("SELECT id FROM tasks WHERE name = 'My Task'").get();
    ...
    it("should save a task with subtasks", async () => {
      ...
      `);
      const task = db.prepare("SELECT id FROM tasks WHERE name = 'Task with Subtasks'").get();

      const subtaskStmt = db.prepare("INSERT INTO subtasks (task_id, name) VALUES (?, ?)");
    ...
    it("should save a task with a custom name", async () => {
      ...
      `);
      const task = db.prepare("SELECT id FROM tasks WHERE name = 'Original Name'").get();

      const template = await saveTemplateFromTask(task.id, { name: "Custom Name" });
    ...
    it("should save a task with a category", async () => {
      ...
      `);
      const task = db.prepare("SELECT id FROM tasks WHERE name = 'Task for Category'").get();
