...
export async function updateSkill(
  id: number,
  userId: number,
  updates: Partial<Omit<CreateUserSkillInput, 'user_id'>>
...
> {
  try {
    ...
    const skillKeywords: Record<string, string[][]> = {
      design: [
        ['design', 'ui', 'ux', 'interface', 'prototype', 'mockup', 'wireframe'],
      ],
      development: [
        [
          'code',
      ...
      writing: [
        ['write', 'document', 'content', 'copy', 'blog', 'article', 'report'],
      ],
      leadership: [
        ['lead', 'manage', 'team', 'coordinate', 'organize', 'mentor', 'guide'],
      ],
      planning: [
        ['plan', 'schedule', 'organize', 'strategy', 'roadmap', 'timeline'],
      ],
      communication: [
        ['email', 'present', 'meeting', 'call', 'discuss', 'talks'],
      ],
      'problem-solving': [
        ['debug', 'fix', 'solve', 'troubleshoot', 'issue', 'bug'],
      ],
      marketing: [['campaign', 'promote', 'advert', 'seo', 'social', 'growth']],
      sales: [['sell', 'pitch', 'demo', 'client', 'customer', 'proposal']],
      finance: [
        ['budget', 'cost', 'invoice', 'payment', 'pricing', 'financial'],
      ],
      'project-management': [
        ['project', 'milestone', 'deliverable', 'scope', 'deadline'],
      ],
      analytical: [['analyze', 'metrics', 'kpi', 'report', 'insight', 'data']],
      creative: [['create', 'brainstorm', 'innovate', 'concept', 'idea']],
      technical: [
        ['setup', 'configure', 'deploy', 'integration', 'automation'],
      ],
    };

    const recommendations = Object.entries(skillKeywords).map(
      ([skill, keywords]) => {
...
export async function getSkillStatistics(userId: number): Promise<{
  ...
  try {
    ...
    const skillDistribution = Array(5)
      .fill(0)
      .map((_, i) => ({
        level: i + 1,
    ...
    logError(
      'Failed to get skill statistics',
      undefined,
      error instanceof Error ? error : new Error(String(error))
    );
    return {
