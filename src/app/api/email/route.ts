  ...
  try {
    ...
    for (const reminder of reminders) {
      try {
        ...
        const user = getDb()
          .prepare(
            'SELECT u.email, t.user_id FROM users u JOIN tasks t ON u.id = t.user_id WHERE t.id = ?'
          )
          .get(reminder.task_id) as
          { email: string; user_id: number } | undefined;

        if (user?.email) {
          // Get task details
          const task = getDb()
            .prepare(
              'SELECT name, description, deadline, priority FROM tasks WHERE id = ?'
            )
            .get(reminder.task_id) as
            | {
                name: string;
