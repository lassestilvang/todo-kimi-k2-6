...
test.describe('Gantt Chart Component', () => {
  ...
  test('should handle empty timeline', async ({ page }) => {
    ...
    await expect(
      page
        .getByText('No tasks', { exact: false })
        .or(page.locator('text=No tasks found').or(page.locator('text=Empty')))
    ).toBeVisible();
  });
  ...
  test('should export Gantt chart', async ({ page }) => {
    ...
    const [download] = await Promise.all([
      ...
      page
        .click('text="Export as CSV"')
        .or(page.click('text="Export as Image"')),
    ]);
