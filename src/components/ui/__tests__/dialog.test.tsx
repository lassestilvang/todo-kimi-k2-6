...
vi.mock('@base-ui/react/dialog', () => {
  ...
  DialogPrimitive.Portal = ({ children }: any) => (
    <div data-testid="dialog-portal">{children}</div>
  );
  DialogPrimitive.Backdrop = ({ children }: any) => (
  ...
  DialogPrimitive.Close = ({ children }: any) => (
    <button data-testid="dialog-close">{children}</button>
  );
  DialogPrimitive.Title = ({ children }: any) => (
    <h2 data-testid="dialog-title">{children}</h2>
  );
  DialogPrimitive.Description = ({ children }: any) => (
    <p data-testid="dialog-description">{children}</p>
  );
  DialogPrimitive.Group = ({ children }: any) => (
    <div data-testid="dialog-group">{children}</div>
  );
  DialogPrimitive.TitlePrimitive = ({ children }: any) => <h2>{children}</h2>;
  DialogPrimitive.DescriptionPrimitive = ({ children }: any) => (
    <p>{children}</p>
  );
...
describe('DialogFooter Component', () => {
  ...
  it('applies custom className', () => {
    const { container } = render(
      <DialogFooter className="custom-footer-class">Content</DialogFooter>
    );
    expect(container.firstChild).toBeTruthy();
  });
});
