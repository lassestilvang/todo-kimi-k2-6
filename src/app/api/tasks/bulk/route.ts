...
export async function POST(request: NextRequest) {
  try {
    ...
    logInfo(`Bulk operation completed: ${action}`, {
...
async function bulkAssignTasks(
  ...
  const placeholders = taskIds.map(() => '?').join(',');
  const result = db
    .prepare(`UPDATE tasks SET assignee_id = ? WHERE id IN (${placeholders})`)
...
async function bulkMoveTasks(
  ...
  const placeholders = taskIds.map(() => '?').join(',');
  const result = db
    .prepare(`UPDATE tasks SET list_id = ? WHERE id IN (${placeholders})`)
...
async function bulkUpdatePriority(
  db: Database,
  taskIds: number[],
  priority: 'critical' | 'high' | 'medium' | 'low' | 'none' | undefined
): Promise<{ updated: number }> {
  if (!priority) {
  ...
  const placeholders = taskIds.map(() => '?').join(',');
  const result = db
    .prepare(`UPDATE tasks SET priority = ? WHERE id IN (${placeholders})`)
    .run(priority, ...taskIds) as DbRunResult;

  return { updated: result.changes };
}

async function bulkUpdateLabels(
  db: Database,
  taskIds: number[],
  labelIds: number[]
): Promise<{ updated: number }> {
  ...
  db.prepare(`DELETE FROM task_labels WHERE task_id IN (${placeholders})`).run(
    ...taskIds
  );
  ...
  if (labelIds.length > 0) {
    ...
    const stmt = db.prepare(
      `INSERT OR REPLACE INTO task_labels (task_id, label_id) VALUES (?, ?)`
    );
