...
export async function enableCalendarSync(
  ...
  if (existing) {
    ...
    ).run(accessToken, refreshToken, expiresAt, tenantId ?? null, userId, provider);
  } else {
    db.prepare(
      "INSERT INTO calendar_sync (user_id, provider, access_token, refresh_token, expires_at, enabled, tenant_id) VALUES (?, ?, ?, ?, ?, 1, ?)"
    ).run(userId, provider, accessToken, refreshToken, expiresAt, tenantId ?? null);
  }
 ...
 */
export async function disableCalendarSync(userId: number, provider: "google" | "outlook"): Promise<boolean> {
  const db = getDb();
  const result = db
    .prepare("UPDATE calendar_sync SET enabled = 0 WHERE user_id = ? AND provider = ?")
    .run(userId, provider);
  return result.changes > 0;
}