...
export async function createPublicShare(
  ...
  const existing = db
    .prepare(
      'SELECT id FROM task_shares WHERE task_id = ? AND share_token IS NULL'
    )
    .get(taskId) as { id: number } | undefined;

  if (existing) {
    db.prepare(
      'UPDATE task_shares SET share_token = ?, permission = ? WHERE id = ?'
    ).run(token, permission, existing.id);
  } else {
    // Insert new share with token
    db.prepare(
      'INSERT INTO task_shares (task_id, permission, share_token) VALUES (?, ?, ?)'
...
export async function getTaskShares(
  taskId: number
): Promise<Array<TaskShare & { user?: User; share_token?: string }>> {
  const db = getDb();
...
export async function getShareByToken(
  token: string
): Promise<(TaskShare & { task_id: number }) | null> {
  const db = getDb();
  return db
    .prepare(
      'SELECT id, task_id, user_id, permission, created_at FROM task_shares WHERE share_token = ?'
    )
