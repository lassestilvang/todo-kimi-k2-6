...
describe("Providers Comprehensive Coverage", () => {
  ...
  describe("KeywordParser - parseTask", () => {
    it("should parse task with critical priority", async () => {
      ...
      const result = await parser.parseTask({ text: "Urgent task needs to be done" });
    ...
    it("should parse task with medium priority", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Standard procedure task" });
      expect(result.priority).toBe("medium");
    });

    it("should parse task with low priority", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Optional task for later" });
      expect(result.priority).toBe("low");
    });

    it("should parse task with no priority match", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Random task without priority keywords" });
      expect(result.priority).toBe("none");
    });

    it("should extract tomorrow date", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Schedule meeting for tomorrow" });
    ...
    it("should extract next week date", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Plan weekend activities" });
      expect(result.suggested_date).toBeDefined();
    });
    ...
    it("should extract duration for call", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Important call with team" });
      expect(result.estimated_duration).toBe(30);
    });
    ...
    it("should extract duration for report", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Create quarterly report" });
      expect(result.estimated_duration).toBe(120);
    });
    ...
    it("should extract custom recurring from every X days", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "every 3 days check emails" });
    ...
    it("should extract deadline from date format", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Task deadline: 2024-03-15" });
      expect(result.deadline).toBe("2024-03-15");
    });

    it("should extract deadline from due format", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Due 2024-05-20 for submission" });
    ...
    it("should extract list_name from context lists when available", async () => {
      ...
      const result = await parser.parseTask({ text: "Add to Work project", context });
    ...
    it("should extract list_name from emoji in context", async () => {
      ...
      const result = await parser.parseTask({ text: "Gym session 💪", context });
    ...
    it("should clean task name by removing prefixes", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "Create a task for project work" });
    ...
    it("should clean task name by removing schedule prefix", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "schedule meeting tomorrow" });
    ...
    it("should parse in X days format", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "in 5 days complete project" });
      expect(result.suggested_date).toBeDefined();
    });

    it("should parse in X weeks format", async () => {
      const parser = new KeywordParser();
      const result = await parser.parseTask({ text: "in 2 weeks plan vacation" });
      expect(result.suggested_date).toBeDefined();
    });
  });
  ...
  describe("KeywordParser - generateInsights", () => {
    ...
    it("should analyze overdue tasks", async () => {
      ...
      const tasks = [
        { id: 1, name: "Overdue 1", completed: false, priority: "high", deadline: pastDate },
  ...
  describe("OpenAIProvider - Error Handling", () => {
    it("should throw error when OPENAI_API_KEY is not configured", async () => {
      ...
      await expect(provider.parseTask({ text: "test task" })).rejects.toThrow("OPENAI_API_KEY not configured");
    });
  });
  ...
  describe("OpenAIProvider - parseTaskStream", () => {
    it("should fallback to keyword parser when OPENAI_API_KEY is not set", async () => {
      ...
      const provider = new OPI();
      const onChunk = vi.fn();
      const result = await provider.parseTaskStream({ text: "urgent task" }, onChunk);
  ...
  describe("ClaudeProvider - Error Handling", () => {
    it("should throw error when ANTHROPIC_API_KEY is not configured", async () => {
      ...
      await expect(provider.parseTask({ text: "test task" })).rejects.toThrow("ANTHROPIC_API_KEY not configured");
    });
  });
  ...
  describe("AIManager - parseTask", () => {
    it("should return cached result when available", async () => {
      ...
      const manager = new providers.AIManager();
      // Set cache
      providers.aiCache.set("parse:test task", { name: "cached", priority: "none", provider: "keyword-parser" });
  ...
  describe("AIManager - generateTasksFromNotes", () => {
    ...
    it("should handle AI generateInsights throwing error", async () => {
      ...
      const mockProvider = {
        ...
        generateInsights: vi.fn(() => { throw new Error("API failed"); }),
      };
  ...
  describe("AIManager - clearCache", () => {
    it("should clear the AI cache", async () => {
      vi.resetModules();
      const providers = await import("../providers");
      providers.aiCache.set("parse:test", { name: "test", priority: "none", provider: "keyword-parser" });
  ...
  describe("AIManager - parseEditCommand", () => {
    it("should parse complete command with task name", async () => {
      ...
      const result = await manager.parseEditCommand("complete Task Alpha", {
        tasks: [{ id: 1, name: "Task Alpha", completed: false, priority: "high" }],
      });
    ...
    it("should parse complete command with mark done", async () => {
      ...
      const result = await manager.parseEditCommand("mark done Task Beta", {
        tasks: [{ id: 2, name: "Task Beta", completed: false, priority: "medium" }],
      });
      expect(result.action).toBe("complete");
    ...
    it("should parse delete command", async () => {
      ...
      const result = await manager.parseEditCommand("delete Gamma", {
        tasks: [{ id: 3, name: "Task Gamma", completed: false, priority: "low" }],
      });
    ...
    it("should parse delete command with remove keyword", async () => {
      ...
      const result = await manager.parseEditCommand("remove Delta", {
        tasks: [{ id: 4, name: "Task Delta", completed: false, priority: "none" }],
      });
      expect(result.action).toBe("delete");
    ...
    it("should parse priority change command", async () => {
      ...
      const providers = await import("../providers");
      const manager = new providers.AIManager();
      const result = await manager.parseEditCommand("change Task Epsilon to critical", {
        tasks: [{ id: 5, name: "Task Epsilon", completed: false, priority: "medium" }],
      });
    ...
    it("should parse set priority command", async () => {
      ...
      const result = await manager.parseEditCommand("set Task Zeta to high", {
        tasks: [{ id: 6, name: "Task Zeta", completed: false, priority: "low" }],
      });
      expect(result.action).toBe("prioritize");
    ...
    it("should parse add label command", async () => {
      ...
      const result = await manager.parseEditCommand("add urgentlabel to Task Eta", {
        tasks: [{ id: 7, name: "Task Eta", completed: false, priority: "medium" }],
      });
    ...
    it("should parse move task to list command (not schedule)", async () => {
      ...
      const result = await manager.parseEditCommand("postpone Task Theta to tomorrow", {
        tasks: [{ id: 8, name: "Task Theta", completed: false, priority: "medium" }],
      });
    ...
    it("should parse schedule command with date", async () => {
      ...
      const providers = await import("../providers");
      const manager = new providers.AIManager();
      const result = await manager.parseEditCommand("schedule Iota for tomorrow", {
        tasks: [{ id: 9, name: "Task Iota", completed: false, priority: "medium" }],
      });
      expect(result.action).toBe("schedule");
    ...
    it("should parse postpone command", async () => {
      ...
      const providers = await import("../providers");
      const manager = new providers.AIManager();
      const result = await manager.parseEditCommand("postpone Kappa to monday", {
        tasks: [{ id: 10, name: "Task Kappa", completed: false, priority: "medium" }],
      });
      expect(result.action).toBe("schedule");
    ...
    it("should parse search command", async () => {
      ...
      const providers = await import("../providers");
      const manager = new providers.AIManager();
      const result = await manager.parseEditCommand("search for important tasks", {
        tasks: [],
      });
    ...
    it("should return edit command for unrecognized input", async () => {
      ...
      const providers = await import("../providers");
      const manager = new providers.AIManager();
      const result = await manager.parseEditCommand("something random that doesn't match", {
    ...
    it("should handle move command where task is not found in context", async () => {
      ...
      const result = await manager.parseEditCommand("put Task Kappa to Work list", {
        tasks: [{ id: 10, name: "Task Kappa", completed: false, priority: "medium" }],
      });
    ...
    it("should handle schedule match when date parse fails", async () => {
      ...
      const result = await manager.parseEditCommand("schedule Lambda for invalid date", {
        tasks: [{ id: 11, name: "Task Lambda", completed: false, priority: "medium" }],
      });
    ...
    it("should handle AI provider parseEditCommand error with continue", async () => {
      ...
      const mockProvider = {
        ...
        parseTask: vi.fn(),
        parseEditCommand: vi.fn(() => { throw new Error("API Error"); }),
        generateInsights: vi.fn(),
        generateTasksFromNotes: vi.fn(),
      };
      ...
      const result = await manager.parseEditCommand("random unrecognized command", {
        tasks: [{ id: 1, name: "Task", completed: false, priority: "medium" }],
      });
    ...
    it("should return AI provider result for parseEditCommand", async () => {
      ...
      const mockProvider = {
        ...
        parseTask: vi.fn(),
        parseEditCommand: vi.fn().mockResolvedValue({ action: "complete", taskId: 99 }),
        generateInsights: vi.fn(),
        generateTasksFromNotes: vi.fn(),
      };
      ...
      const result = await manager.parseEditCommand("some command", {
        tasks: [{ id: 99, name: "Other Task", completed: false, priority: "medium" }],
      });
  ...
  describe("parseEditCommand Cache Hit", () => {
    it("should return cached result when available for edit command", async () => {
      ...
      const cachedEdit = { action: "complete", taskId: 1, provider: "keyword-parser" };
  ...
  describe("AICache Expiration via aiCache singleton", () => {
    it("should return null for expired cache entry using direct cache manipulation", async () => {
      ...
      // Set entry
      providers.aiCache.set("test-key", { value: "test", provider: "keyword-parser" });
