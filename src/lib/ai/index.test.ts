...
describe('AI Module', () => {
  describe('parseTaskInput', () => {
    ...
    it('should detect critical priority', async () => {
      const result = await parseTaskInput({
        text: 'URGENT: Fix the production bug',
      });
    ...
    it('should detect high priority', async () => {
      const result = await parseTaskInput({
        text: 'This is IMPORTANT for the project',
      });
    ...
    it('should detect low priority', async () => {
      const result = await parseTaskInput({
        text: 'Low priority task for later',
      });
    ...
    it('should parse deadline with deadline: prefix', async () => {
      const result = await parseTaskInput({
        text: 'Submit report deadline: 2024-12-31',
      });
  ...
  describe('generateTaskInsights', () => {
    ...
    it('should provide positive tips for high completion rate', async () => {
      ...
      const insights = await generateTaskInsights(tasks);
      expect(
        insights.productivity_tips.some(t => t.includes('excellent'))
      ).toBe(true);
    });
  ...
  describe('parseNaturalLanguageTask', () => {
    it('should extract list_name when work-related keywords present', async () => {
      ...
      const result = await parseNaturalLanguageTask(
        'Schedule a meeting at the office for tomorrow'
      );
      expect(result.name).toBeDefined();
    ...
    it('should return empty labels when no list keywords found', async () => {
      const { parseNaturalLanguageTask } = await import('./index');
      const result = await parseNaturalLanguageTask(
        'Some random task without context'
      );
    ...
    it('should extract work-related list', async () => {
      const { parseNaturalLanguageTask } = await import('./index');
      const result = await parseNaturalLanguageTask(
        'Review the project with the team'
      );
