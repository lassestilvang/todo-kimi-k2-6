...
import { Brain, AlertCircle, CheckCircle, TrendingDown, TrendingUp } from "lucide-react";
...
export function CognitiveLoadIndicator({ className }: CognitiveLoadIndicatorProps) {
  ...
  const completionRate = localLoad.tasks > 0
    ? (localLoad.completed / localLoad.tasks) * 100
    : 0;

  const loadLevel = completionRate < 50 ? "overloaded" : completionRate < 75 ? "attention" : "healthy";
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              ...
              {loadLevel === "overloaded" && <AlertCircle className="h-4 w-4 text-red-500" />}
              {loadLevel === "attention" && <AlertCircle className="h-4 w-4 text-amber-500" />}
              {loadLevel === "healthy" && <CheckCircle className="h-4 w-4 text-green-500" />}
            </div>

            <div className="text-sm text-muted-foreground mb-2">
              {analysis?.loadTrend === "increasing" && <TrendingUp className="h-3 w-3 inline mr-1" />}
              {analysis?.loadTrend === "decreasing" && <TrendingDown className="h-3 w-3 inline mr-1" />}
            ...
            <div className="space-y-2">
              <div>
                <span className="text-xs text-muted-foreground">
                  Tasks: {localLoad.tasks} ({completionRate.toFixed(0)}% completed)
                </span>
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                ...
                <div>
                  <p className="text-lg font-bold">{analysis?.avgFocusBlocks ?? 2}</p>
                  <p className="text-xs text-muted-foreground">Focus Blocks</p>
                </div>
                <div>
                  <p className="text-lg font-bold">{analysis?.avgInterruptions ?? 1}</p>
