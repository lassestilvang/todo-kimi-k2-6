...
describe('Outlook Calendar Sync', () => {
  ...
  describe('refreshAccessTokenIfNeeded', () => {
    it('returns existing token when not expired', async () => {
      const { refreshAccessTokenIfNeeded } = await import('../outlook-calendar');
      const sync: any = {
        id: 1,
    ...
    it('returns null when client credentials missing', async () => {
      ...
      const originalClientId = process.env.OUTLOOK_CLIENT_ID;
      const originalClientSecret = process.env.OUTLOOK_CLIENT_SECRET;
      delete (process.env as any).OUTLOOK_CLIENT_ID;
      delete (process.env as any).OUTLOOK_CLIENT_SECRET;
      ...
      // Restore env vars
      if (originalClientId) process.env.OUTLOOK_CLIENT_ID = originalClientId;
      if (originalClientSecret) process.env.OUTLOOK_CLIENT_SECRET = originalClientSecret;
    });
