...
export interface KnowledgeEntry {
  ...
  title: string;
  content: string;
  type: "lesson_learned" | "best_practice" | "tip" | "insight" | "tool" | "template";
  category?: string;
...
export async function createKnowledgeEntry(
  ...
  return db.prepare("SELECT * FROM knowledge_entries WHERE id = ?").get(result.lastInsertRowid) as KnowledgeEntry;
}
 ...
 */
export async function getKnowledgeEntryWithEvolution(id: number, userId: number): Promise<EvolutionTimeline | null> {
  const db = getDb();
  ...
  const evolutionSteps = db.prepare(`
  ...
  const improvementScore = Math.min(100, evolutionSteps.length * 10 + entry.confidence * 100);
 ...
 */
export async function getKnowledgeStats(userId: number): Promise<KnowledgeStats> {
  const db = getDb();
  ...
  const totalEntries = db.prepare(`SELECT COUNT(*) as count FROM knowledge_entries WHERE user_id = ?`).get(userId) as { count: number };
  ...
  stats.forEach(s => { byType[s.type] = s.count; });

  const byCategory: Record<string, number> = {};
  categories.forEach(c => { byCategory[c.category] = c.count; });
 ...
 */
export async function extractLessonsLearned(userId: number): Promise<LessonLearned[]> {
  const db = getDb();
...
export async function addEvolutionStep(
  ...
  const existing = db.prepare("SELECT * FROM knowledge_entries WHERE id = ? AND user_id = ?").get(entryId, userId) as KnowledgeEntry | undefined;
  if (!existing) {
    throw new Error("Knowledge entry not found");
  }

  const versionCount = db.prepare("SELECT COUNT(*) as count FROM evolution_steps WHERE entry_id = ?").get(entryId) as { count: number };
  ...
  return db.prepare("SELECT * FROM evolution_steps WHERE id = ?").get(result.lastInsertRowid) as EvolutionStep;
}

/**
 * Generate knowledge timeline view
 */
export async function getKnowledgeTimeline(userId: number, months = 6): Promise<{
  month: string;
  entries: number;
  lessons: number;
}[]> {
...
export async function getEvolutionMetrics(userId: number): Promise<{
  ...
  const entriesWithEvolution = db.prepare(`
  ...
  const totalEvolutions = db.prepare("SELECT COUNT(*) as count FROM evolution_steps").get() as { count: number };

  const latestEvolution = db.prepare(`
    SELECT MAX(created_at) as latest FROM evolution_steps
  `).get() as { latest: string };

  const avgVersionsPerEntry = entriesWithEvolution.count > 0
    ? Math.round(totalEvolutions.count / entriesWithEvolution.count * 10) / 10
    : 0;
