...
export function DecisionAnalytics({ taskId }: DecisionAnalyticsProps) {
  ...
  const [metric, setMetric] = useState<'trend' | 'rating' | 'outcome' | 'type'>(
    'trend'
  );
  ...
  const analytics = useMemo(() => {
    ...
    const decisionsByWeek = filteredDecisions.reduce(
      (acc, d) => {
    ...
    const typeWithOutcomes = (type: string) =>
      filteredDecisions.filter(d => d.decision_type === type && d.outcome)
        .length;

    const typeDistribution = decisionTypes.map(type => {
      const typeDecisions = filteredDecisions.filter(
      ...
      return {
        ...
        completionRate:
          typeDecisions.length > 0
            ? (typeWithOutcomes(type.value) / typeDecisions.length) * 100
            : 0,
        color: type.color,
        icon: type.icon,
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-4 items-center">
        ...
        <Select
          value={metric}
          onValueChange={v =>
            setMetric(v as 'trend' | 'rating' | 'outcome' | 'type')
          }
        >
          <SelectTrigger className="w-40">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <p
                className={`text-2xl font-bold ${analytics.avgRating > 0.5 ? 'text-green-500' : analytics.avgRating < -0.5 ? 'text-red-500' : 'text-amber-500'}`}
              >
                {analytics.avgRating.toFixed(2)}
              </p>
              <BarChart3
                className={`h-4 w-4 ${analytics.avgRating > 0.5 ? 'text-green-500' : analytics.avgRating < -0.5 ? 'text-red-500' : 'text-amber-500'}`}
              />
            </div>
          </CardContent>
        </Card>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="relative h-2 bg-muted rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(item.count / analytics.total) * 100 || 0}%`,
                        }}
                      />
                    </div>
          ...
          <ul className="space-y-2 text-sm">
            {analytics.avgRating < 0.33 && (
              <li className="flex items-center gap-2">
                ...
                <span>
                  Your decision outcomes could be better. Consider recording
            ...
            {analytics.typeDistribution.some(t => t.avgRating < 0) && (
              <li className="flex items-center gap-2">
                <AlertCircle className="h-4 w-4 text-amber-500" />
                <span>
                  Some decision types are underperforming. Review your approach
                  to those decisions.
                </span>
              </li>
            )}
            {analytics.completionRate < 50 && (
              <li className="flex items-center gap-2">
                <AlertCircle className="h-4 w-4 text-amber-500" />
                <span>
                  Record outcomes for more decisions to get better insights.
                </span>
              </li>
            )}
            {analytics.avgRating > 0.5 && analytics.completionRate > 70 && (
              <li className="flex items-center gap-2 text-green-600">
                ...
                <span>
                  Excellent decision-making! Keep tracking outcomes for
