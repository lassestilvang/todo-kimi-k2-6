...
describe("TaskSubtasks Component - Comprehensive Tests", () => {
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle many subtasks without performance issues", () => {
      const manySubtasks = Array.from({ length: 20 }, (_, i) => `Subtask number ${i + 1}`);
    ...
    it("should handle special characters in subtask names", () => {
      ...
      expect(screen.getByDisplayValue("Task with <html> & chars")).toBeInTheDocument();
      expect(screen.getByDisplayValue("Emoji 🎉 test")).toBeInTheDocument();
    });

    it("should handle long subtask names", () => {
      const longName = "This is a very long subtask name that tests how the component handles lengthy text inputs without breaking the layout or causing overflow issues";
      const props = {
        ...defaultProps,
        subtasks: [longName],
      };
