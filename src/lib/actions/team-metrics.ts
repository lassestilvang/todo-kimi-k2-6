...
export async function getTeamVelocityReport(
  ...
  const avgCompletionRate =
    teamMembers.reduce((sum, m) => sum + m.completion_rate, 0) /
...
async function getVelocityTrend(
  ...
  const currentParams = workspaceId
    ? [workspaceId, workspaceId, currentPeriodStart.toISOString()]
  ...
  const previousParams = workspaceId
    ? [
        workspaceId,
        workspaceId,
  ...
  const trend = Math.round(
    ((currentResult.count - previousResult.count) / previousResult.count) * 100
  );
...
async function getCapacityUtilization(
  ...
  const trackedSeconds = timeResults.reduce(
