...
describe('Time Actions', () => {
  ...
  describe('updateTimeEntry', () => {
    ...
    it('should throw for non-existent entry', async () => {
      await expect(
        updateTimeEntry(99999, { description: 'test' })
      ).rejects.toThrow('Time entry not found');
    });

    it('should throw for invalid entry ID', async () => {
      await expect(
        updateTimeEntry(-1, { description: 'test' })
      ).rejects.toThrow('Invalid time entry ID');
    });
    ...
    it('should throw when no fields to update', async () => {
      ...
      await expect(updateTimeEntry(entry.id, {})).rejects.toThrow(
        'No fields to update'
      );
    });
