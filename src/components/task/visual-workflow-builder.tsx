...
export function VisualWorkflowBuilder({ className }: { className?: string }) {
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (over.id === 'palette-drop-zone') {
      const newType = active.data?.current?.type as
        keyof typeof ACTION_NODES | keyof typeof TRIGGER_NODES;
      const subtype = active.data?.current?.subtype;
      const typeStr = String(newType || '');
      const nodeList = typeStr.startsWith('action')
        ? ACTION_NODES
        : TRIGGER_NODES;
      const node = (nodeList as WorkflowNode[]).find(
        n => n.id === subtype || n.subtype === subtype
      );
      if (node) {
        setWorkflowNodes([
          ...workflowNodes,
  ...
  const SortableWorkflowNode = ({
    ...
    const Icon =
      node.icon || (node.type === 'trigger' ? AlertCircle : CheckCircle2);
