...
export function useCognitiveLoad() {
  ...
  const fetchAnalysis = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/enhanced-productivity/cognitive-load?days=7");
...
export function useEnergyBudget() {
  ...
  const updateProfile = useCallback(async (profileData: Partial<EnergyProfile>) => {
    const res = await fetch("/api/enhanced-productivity/energy-budget", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  return { profile, budget, loading, updateProfile, logEnergy, refetch: fetchData };
}
...
export function useExternalTasks(status: string = "pending") {
  ...
  const fetchTasks = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/enhanced-productivity/external-tasks?status=${status}`);
      const data = await res.json();
      setTasks(data.tasks);
    } catch (error) {
...
export function useDecisionShadow() {
  ...
  const fetchAnalysis = useCallback(async (limit: number = 20) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/enhanced-productivity/decisions?limit=${limit}`);
      const data = await res.json();
      setAnalysis(data);
    } catch (error) {
...
export function useMoodTracking() {
  ...
  const fetchRecommendations = useCallback(async (date?: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/enhanced-productivity/mood?date=${targetDate}`);
      const data = await res.json();
  ...
  return { recommendations, loading, logMood, getRecommendations, refetch: fetchRecommendations };
}
...
export function useEnhancedProductivity() {
  ...
  return {
    ...
    decisionShadow,
    moodTracking,
    loading: cognitiveLoad.loading || energyBudget.loading ||
             externalTasks.loading || decisionShadow.loading || moodTracking.loading,
    refreshAll: () => {
