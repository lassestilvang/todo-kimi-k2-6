...
export async function GET(request: NextRequest) {
  ...
  switch (feature) {
    ...
    case 'persona':
      ...
      const { analyzeUserPersona, getPersonaRecommendations } =
        await import('@/lib/actions/productivity-personas');
      try {
        const userId = middleware.auth?.userId ?? 1;
  ...
  switch (action) {
    ...
    case 'predict':
      ...
      try {
        ...
        const result = await aiManager.predictTaskDuration(
          body.task,
          body.context
        );
