...
export async function PATCH(
  ...
  if (role === 'owner') {
    const existingOwner = db
      .prepare(
        "SELECT user_id FROM workspace_users WHERE workspace_id = ? AND role = 'owner'"
      )
      .get(workspaceId) as { user_id: number } | undefined;

    if (existingOwner && existingOwner.user_id !== userId) {
      db.prepare(
        "UPDATE workspace_users SET role = 'admin' WHERE workspace_id = ? AND user_id = ?"
...
export async function DELETE(
  ...
  // Delete member
  db.prepare(
    'DELETE FROM workspace_users WHERE id = ? AND workspace_id = ?'
