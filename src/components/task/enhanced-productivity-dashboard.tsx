...
export function EnhancedProductivityDashboard({
  className,
}: EnhancedProductivityDashboardProps) {
    ...
    <div className={`space-y-6 ${className}`}>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {cognitiveLoad.analysis?.loadTrend === 'stable'
                ? 'Stable'
                : cognitiveLoad.analysis?.loadTrend === 'increasing'
                  ? 'Rising'
                  : 'Falling'}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {energyBudget.budget?.balance ?? 100}/
              {energyBudget.budget?.dailyLimit ?? 100}
            </div>
            <Progress
              value={
                ((energyBudget.budget?.balance ?? 100) /
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Avg rating:{' '}
              {decisionShadow.analysis?.avgOutcomeRating?.toFixed(1) ?? 'N/A'}
            </p>
          </CardContent>
        </Card>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {cognitiveLoad.analysis?.recommendations?.map(
                ...
                {!cognitiveLoad.analysis?.recommendations && (
                  <p className="text-muted-foreground">
                    Load more data for recommendations
                  </p>
                )}
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {decisionShadow.analysis?.patternAnalysis?.map(
                  (
                    pattern: { pattern: string; recommendation: string },
                    idx: number
                  ) => (
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-lg">
                      {energyBudget.profile?.wake_hour ?? 7}:00
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium">Sleep Hour</p>
                    <p className="text-lg">
                      {energyBudget.profile?.sleep_hour ?? 23}:00
                    </p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>
                      {energyBudget.budget?.balance ?? 100} points used
                    </span>
                    <span>
                      {energyBudget.budget?.dailyLimit ?? 100} points budget
                    </span>
                  </div>
                </div>
                  ...
                  externalTasks.tasks.map(task => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          From: {task.external_app_type} • Confidence:{' '}
                          {task.confidence}%
                        </p>
                      </div>
                      <Button
                        onClick={() => externalTasks.convertToTask(task.id)}
                      >
                        Convert to Task
                      </Button>
                    </div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="text-center p-4 bg-muted rounded-lg">
                    <p className="text-2xl font-bold">
                      {Math.round(
                        decisionShadow.analysis?.avgOutcomeRating ?? 0
                      )}
                      /5
                    </p>
                ...
                <div>
                  ...
                  {decisionShadow.analysis?.patternAnalysis?.map(
                    (
                      ...
                      <div
                        key={idx}
                        className="border-l-2 border-purple-200 pl-4 mb-3"
                      >
