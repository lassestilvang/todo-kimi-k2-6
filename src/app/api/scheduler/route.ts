...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const analysis = await calculateOptimalSchedule(userId || middlewareResult.auth.userId, taskIds, {
      date: date,
      durationPreference: durationPreference || "balanced",
    });

    // Get energy profile
    const energyProfile = await getUserEnergyProfile(userId || middlewareResult.auth.userId);
    ...
    const message = error instanceof Error ? error.message : "Failed to calculate schedule";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to fetch energy profile";
    return errorResponse(message, 500);
  }
}