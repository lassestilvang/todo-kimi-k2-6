...
const COMMANDS: Record<
  string,
  { pattern: RegExp; intent: string; action: string }
> = {
  create_task: {
    pattern:
      /create (?:a |new |)?task (?:called |named |)"|add (?:a |new |)"|make (?:a |new |)"/i,
    intent: 'create_task',
    action: 'create',
  },
  complete_task: {
    pattern:
      /complete (?:the |my |)"|mark (?:as )?(?:complete |done )"|" completed/i,
    intent: 'complete_task',
    action: 'complete',
  },
  search_tasks: {
    pattern:
      /find (?:tasks |my )?"|search for (?:tasks |my )?"|show (?:me )?tasks( that)?/i,
    intent: 'search_tasks',
    action: 'search',
  },
  list_tasks: {
  ...
  today_tasks: {
    pattern:
      /what (?:are )?my (?:today |tasks for today)"|tasks (?:due )?today/i,
    intent: 'today_tasks',
    action: 'today',
  },
  help: {
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            <Button
              ...
              size="sm"
              onClick={() =>
                handleVoiceCommand('create a task called review documentation')
              }
            >
              Create Task
            </Button>
