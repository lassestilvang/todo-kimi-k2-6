...
const decisionTypes = [
  { value: "priority", label: "Priority Decision", icon: Brain, color: "bg-red-500/10 text-red-600" },
  { value: "approach", label: "Approach Decision", icon: Lightbulb, color: "bg-blue-500/10 text-blue-600" },
  { value: "tool", label: "Tool Selection", icon: Award, color: "bg-green-500/10 text-green-600" },
  { value: "timeline", label: "Timeline Decision", icon: Calendar, color: "bg-amber-500/10 text-amber-600" },
  { value: "allocation", label: "Resource Allocation", icon: Users, color: "bg-purple-500/10 text-purple-600" },
  { value: "cancellation", label: "Cancellation Decision", icon: AlertCircle, color: "bg-gray-500/10 text-gray-600" },
];
...
export function DecisionTracker({ taskName, taskId }: DecisionTrackerProps) {
  ...
  const [editingDecision, setEditingDecision] = useState<DecisionEntry | null>(null);
  ...
  const decisionStats = useMemo(() => {
    ...
    const typeStats = decisionTypes.reduce((acc, type) => {
      const typeDecisions = decisions.filter(d => d.decision_type === type.value);
      const typeRated = typeDecisions.filter(d => d.outcome_rating !== null);
      acc[type.value] = {
        count: typeDecisions.length,
        avgRating: typeRated.reduce((sum, d) => sum + (d.outcome_rating || 0), 0) / (typeRated.length || 1),
      };
      return acc;
  ...
  const handleUpdateOutcome = async (id: number, outcome: string, rating: number | null) => {
    try {
      const response = await fetch("/api/decisions", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const RatingBadge = ({ value }: { value: number | null }) => {
    ...
    const label = value >= 0.5 ? "Positive" : value <= -0.5 ? "Negative" : "Neutral";
    const color = value > 0.33 ? "default" : value < -0.33 ? "destructive" : "secondary";
    return (
      <BadgeComponent className={color === "default" ? "" : color === "destructive" ? "" : ""}>
        {label}
      </BadgeComponent>
    );
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">With Outcomes</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{Math.round(decisionStats.completionRate)}%</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className={`text-2xl font-bold ${getRatingColor(decisionStats.avgRating)}`}>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {decisionTypes.map((type) => {
              const stats = decisionStats.typeStats[type.value] || { count: 0, avgRating: 0 };
              const Icon = type.icon;
              return (
                <div key={type.value} className="border rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-2">
                    <Icon className="h-4 w-4 text-muted-foreground" />
                    <BadgeComponent className={type.color}>{type.label}</BadgeComponent>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.decision_type} onValueChange={(v) => setFormData({ ...formData, decision_type: v as any })}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
      ...
      {filteredDecisions.length === 0 ? (
          ...
          <CardContent className="pt-6">
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mb-4">
                Start recording your important decisions to track your decision-making patterns
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-3">
          {filteredDecisions.map((decision) => {
            const Icon = decisionTypes.find(t => t.value === decision.decision_type)?.icon || Brain;
            return (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <BadgeComponent>
                        {decisionTypes.find(t => t.value === decision.decision_type)?.label || "Decision"}
                      </BadgeComponent>
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <Textarea
                              ...
                              onBlur={(e) => handleUpdateOutcome(decision.id, e.target.value, null)}
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              defaultValue={decision.outcome_rating?.toString() || ""}
                              onValueChange={(v) => {
                                const rating = v ? parseFloat(v) : null;
                                handleUpdateOutcome(decision.id, decision.outcome || "", rating);
                              }}
                            >
                              <SelectTrigger>
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="-0.5">🔶 -0.5 (Fair)</SelectItem>
                                <SelectItem value="0">⚪ 0 (Neutral)</SelectItem>
                                <SelectItem value="0.5">🟢 0.5 (Good)</SelectItem>
                                <SelectItem value="1">🟢 1 (Excellent)</SelectItem>
                              </SelectContent>
                            </Select>
                          </div>

                          <Button onClick={() => setEditingDecision(null)}>Done</Button>
                        </div>
                      </DialogContent>
                    </Dialog>
                  )}
