...
export function TaskContextTab({
  ...
  const getContextStats = (contextType: string) => {
    ...
    return context
      ? { frequency: context.frequency, successRate: context.success_rate }
      : null;
  };
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <Select
          ...
          onValueChange={v => setSelectedContextType(v as string)}
        >
          <SelectTrigger>
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium">
            {contextTypes.find(t => t.value === selectedContextType)?.label ||
              'Context'}
          </label>
          <div className="grid grid-cols-2 gap-2">
            {contextTypes
              .find(t => t.value === selectedContextType)
              ?.options.map(option => (
                <Button
                  key={option.value}
                  variant={
                    selectedContextValue === option.value
                      ? 'default'
                      : 'outline'
                  }
                  size="sm"
      ...
      {contexts.length > 0 && (
        <div className="mt-4 p-3 bg-muted/30 rounded-lg">
          ...
          {contexts.map(context => {
            const type = contextTypes.find(
              t => t.value === context.context_type
            );
            if (!type) return null;
            const Icon = type.icon;
