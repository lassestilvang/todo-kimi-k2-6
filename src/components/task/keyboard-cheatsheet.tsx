...
import { BookOpen, Brain, Zap, BarChart3, Settings, Lightbulb, Layout, Check } from "lucide-react";
...
export function KeyboardCheatsheet({ open, onOpenChange }: KeyboardCheatsheetProps) {
  const shortcuts: ShortcutCategory[] = [
    ...
    },
    {
      ...
      icon: BarChart3,
      items: [
        { keys: "Cmd + 4", description: "Go to Analytics dashboard" },
      ],
    },
    {
      category: "Labs",
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === "/") {
        ...
        if (!activeElement?.closest("input") && !activeElement?.closest("textarea") && !activeElement?.closest(".prose")) {
          e.preventDefault();
          onOpenChange(true);
        }
      }
  ...
  const formatKeys = (keys: string): string[] => {
    return keys.split(" + ").map((key) => {
      const displayKey = key.replace("⌘", "Cmd").replace("⇧", "Shift").replace("⌥", "Option").replace("⌃", "Ctrl");
      return displayKey;
    });
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Speed up your workflow with these keyboard shortcuts. Press the keys in order.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="border-t pt-4 mt-4">
          <div className="text-center text-sm text-muted-foreground">
            <p className="mb-2">
              <span className="font-medium">Pro Tip:</span> Click on any task or field, then type natural language to edit it
            </p>
            <p>
              Examples: "complete meeting with John", "move project task to tomorrow", "set priority high on bug fix"
            </p>
          </div>
        </div>
      </DialogContent>
