...
export async function createWorkspace(
  input: CreateWorkspaceInput & { user_id: number }
): Promise<Workspace> {
  const db = getDb();
  const result = db
    .prepare(
...
export async function getWorkspaceMembers(
  workspaceId: number
): Promise<Array<WorkspaceUser & { email: string; name: string | null }>> {
  const db = getDb();
  return db
    .prepare(
    ...
    WorkspaceUser & { email: string; name: string | null }
  >;
}
...
export async function canUserAccessWorkspace(
  ...
  const result = db
    .prepare(
      'SELECT 1 FROM workspace_users WHERE user_id = ? AND workspace_id = ?'
    )
...
export async function getUserWorkspaceRole(
  ...
  const result = db
    .prepare(
      'SELECT role FROM workspace_users WHERE user_id = ? AND workspace_id = ?'
    )
...
export async function updateUserWorkspaceRole(
  ...
  const db = getDb();
  db.prepare(
    'UPDATE workspace_users SET role = ? WHERE user_id = ? AND workspace_id = ?'
...
export async function transferWorkspaceOwnership(
  ...
  // Add as owner
  db.prepare(
    "INSERT INTO workspace_users (workspace_id, user_id, role) VALUES (?, ?, 'owner')"
  ).run(workspaceId, newOwnerId);
}
