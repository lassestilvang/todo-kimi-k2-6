...
export async function generateProactiveSuggestions(
  ...
  if (completedYesterday.length > 0 && completedToday.length === 0 && currentHour > 12) {
    suggestions.push({
      type: "streak",
  ...
  if (currentHour >= workStartTime && currentHour < workStartTime + 2 && incompleteToday.length > 0) {
    suggestions.push({
      type: "focus",
  ...
  const overdueCritical = tasks.filter(
    (t) => !t.completed && t.priority === "critical" && t.deadline && new Date(t.deadline) < now
  );
...
export function generatePatternNudges(
  ...
  const successRateByHour = userHistory.reduce((acc, entry) => {
  ...
  const peakHours = Object.entries(successRateByHour)
    ...
    .sort(([a], [b]) => successRateByHour[Number(b)].completed / userHistory.length - successRateByHour[Number(a)].completed / userHistory.length)
    .slice(0, 3)
    .map(([hour]) => Number(hour));

  // If it's near a peak hour, suggest starting work
  if (peakHours.includes(currentHour) || peakHours.includes(currentHour - 1)) {
    const incompleteTasks = tasks.filter((t) => !t.completed && t.date === now.toISOString().split("T")[0]);
