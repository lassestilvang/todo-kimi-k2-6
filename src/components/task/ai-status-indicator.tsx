...
export function AIStatusIndicator({
  aiProvider = 'keyword-parser',
  confidenceScore = 0.5,
  className,
}: AIStatusIndicatorProps) {
  const config =
    aiProviderConfig[aiProvider] || aiProviderConfig['keyword-parser'];
  const Icon = config.icon;

  // Only show indicator if confidence is meaningful or it's an AI provider
  const shouldShow =
    confidenceScore > 0.5 ||
    !aiProviderConfig[aiProvider]?.description?.includes('keyword');
