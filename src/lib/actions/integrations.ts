...
export async function syncTasksFromIntegration(
  ...
  const integration = db
    .prepare(
      'SELECT * FROM integrations WHERE id = ? AND user_id = ? AND enabled = 1'
    )
    .get(integrationId, user.id) as
    (Integration & { config: string }) | undefined;
  ...
  switch (integration.type) {
    ...
    default:
      throw new Error(
        `Integration type ${integration.type} not yet implemented`
      );
  }
  ...
  for (const externalTask of externalTasks) {
    ...
    const existingLocalTask = db
      .prepare(
        'SELECT id FROM tasks WHERE user_id = ? AND (tasks.id = ? OR EXISTS (SELECT 1 FROM task_mappings WHERE task_id = tasks.id AND external_task_id = ?))'
      )
      .get(user.id, mapping.local_task_id, externalTask.id) as
      { id: number } | undefined;
...
export async function syncTasksToIntegration(
  ...
  switch (integration.type) {
    ...
    case 'todoist':
      ...
      break;
    case 'github':
      [success, errors] = await exportGitHubIntegration(
        integration,
        mappedTasks
      );
      break;
    case 'slack':
      [success, errors] = await exportSlackIntegration(
        integration,
        mappedTasks
      );
      break;
    case 'notion':
      [success, errors] = await exportNotionIntegration(
        integration,
        mappedTasks
      );
      break;
    case 'linear':
      [success, errors] = await exportLinearIntegration(
        integration,
        mappedTasks
      );
      break;
    default:
      errors = [
        `Integration type ${integration.type} not yet implemented for export`,
      ];
  }
...
export async function updateIntegrationMapping(
  ...
  const mapping = db
    .prepare(
      'SELECT * FROM task_mappings WHERE id = ? AND integration_id IN (SELECT id FROM integrations WHERE user_id = ?)'
    )
    .get(updates.id, user.id) as
    (TaskMapping & { field_mappings: string }) | undefined;

  if (!mapping) {
    throw new Error('Task mapping not found or not accessible');
  }

  const currentMappings: Record<string, string> = mapping.field_mappings
    ? JSON.parse(mapping.field_mappings)
    : {};
  const newMappings: Record<string, string> = updates.field_mappings
    ? {
        ...currentMappings,
        ...(updates.field_mappings as Record<string, string>),
      }
    : currentMappings;
  ...
  return {
    ...
    sync_rules: updates.sync_rules
      ? JSON.stringify(updates.sync_rules)
      : mapping.sync_rules,
  ...
  const statusPromises = integrations.map(async integration => {
    ...
    try {
      const countResult = db
        .prepare(
          'SELECT COUNT(*) as count FROM task_mappings WHERE integration_id = ?'
        )
        .get(integration.id);
      ...
      const lastMapping = db
        .prepare(
          'SELECT last_sync_at FROM task_mappings WHERE integration_id = ? ORDER BY last_sync_at DESC LIMIT 1'
        )
...
export async function disconnectIntegration(
  ...
  db.prepare('DELETE FROM task_mappings WHERE integration_id = ?').run(
    integrationId
  );
...
export async function getUserTaskMappings(
  ...
  let query =
    'SELECT * FROM task_mappings WHERE integration_id IN (SELECT id FROM integrations WHERE user_id = ?)';
