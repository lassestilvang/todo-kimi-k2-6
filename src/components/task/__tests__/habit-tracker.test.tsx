...
vi.mock('lucide-react', () => ({
  Flame: ({ className }: { className?: string }) => (
    <span className={className} data-testid="flame-icon">
      🔥
    </span>
  ),
  ...
  TrendingUp: ({ className }: { className?: string }) => (
    <span className={className} data-testid="trending-icon">
      📈
    </span>
  ),
  Award: ({ className }: { className?: string }) => (
    <span className={className} data-testid="award-icon">
      🏆
    </span>
  ),
  Target: ({ className }: { className?: string }) => (
    <span className={className} data-testid="target-icon">
      🎯
    </span>
  ),
...
describe('HabitTracker Component', () => {
  ...
  describe('getStreakLevel function', () => {
    ...
    it('should show 0 total streak days when no data', async () => {
      ...
      await waitFor(() => {
        ...
        const totalStreakCard = cards.find(c =>
          c.textContent?.includes('Total Streak Days')
        );
  ...
  describe('Task filtering logic', () => {
    it('should identify habit tasks correctly (recurring !== none)', () => {
      const habitTasks = mockTasks.filter(
        t => t.recurring !== 'none' && !t.completed
      );
