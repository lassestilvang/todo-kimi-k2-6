...
export function SmartInbox({ className, lists = [], labels = [], onTaskCreated }: SmartInboxEnhancedProps) {
  ...
  const handleConvert = async (item: SmartInboxItem) => {
    try {
      const taskData = {
        ...
        metadata: { from_smart_inbox: true, source_type: item.source.source_type },
      };
  ...
  const getDaysUntil = (date?: string): number => {
    ...
    const diff = Math.ceil((target.getTime() - today.getTime()) / (1000 * 1000 * 60 * 60 * 24));
    return diff;
  };
          ...
          <div className="grid grid-cols-4 gap-4 mb-4">
            ...
            <div className="text-center p-3 bg-muted/30 rounded-lg">
              <p className="text-2xl font-bold">
                {items.filter(i => i.predicted_priority === "high" || i.predicted_priority === "critical").length}
              </p>
            ...
            <div className="text-center p-3 bg-muted/30 rounded-lg">
              <p className="text-2xl font-bold">
                {items.filter(i => i.suggested_labels && i.suggested_labels.length > 0).length}
              </p>
              ...
              {filteredItems.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <div className="flex gap-2 justify-center">
                    <Button onClick={() => setAutoTriageEnabled(!autoTriageEnabled)}>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              {item.predicted_priority && item.predicted_priority !== "none" && (
                                <Badge className={PRIORITY_COLORS[item.predicted_priority]}>
                            ...
                            {item.ai_reasoning && (
                              <div className="mb-2 p-2 bg-blue-50 rounded border-l-4 border-blue-200">
                                <div className="flex items-start gap-2">
                                  ...
                                  <p className="text-xs text-blue-800">{item.ai_reasoning}</p>
                                </div>
                              </div>
                            )}
                            ...
                            {item.suggested_labels && item.suggested_labels.length > 0 && (
                              <div className="flex items-center gap-1 mb-2">
                                ...
                                {item.suggested_labels.slice(0, 3).map(label => (
                            ...
                            <div className="flex items-center gap-4 text-xs text-muted-foreground">
                              ...
                              <span className={getConfidenceColor(item.source.confidence || 50)}>
                                Confidence: {item.source.confidence || 50}%
                              </span>
                              {item.predicted_due_date && (
                                <span className={getDaysUntil(item.predicted_due_date) < 0 ? "text-red-600" : ""}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    type="date"
                    defaultValue={selectedItem.predicted_due_date || selectedItem.source.due_date || ""}
                    className="mt-1"
                  />
                </div>
                <div>
                  <label className="text-sm font-medium">Priority</label>
                  <Select defaultValue={selectedItem.predicted_priority || selectedItem.source.priority}>
              ...
              {selectedItem.suggested_labels && selectedItem.suggested_labels.length > 0 && (
                <div>
