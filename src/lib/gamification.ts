...
const ACHIEVEMENTS: Record<
  string,
  Omit<Achievement, 'unlockedAt' | 'progress' | 'target'>
> = {
  'first-task': {
    id: 'first-task',
    name: 'First Step',
...
export function calculateAchievements(
  ...
  const { tasks, goals, streakDays, totalCompleted, totalHoursLogged } =
    context;
...
export function calculateSkillPoints(
  ...
  const avgTasksPerDay =
    tasks.filter(t => t.completed).length / Math.max(1, daysActive);
  ...
  const highPriorityCompleted = tasks.filter(
    t => t.completed && (t.priority === 'high' || t.priority === 'critical')
  ).length;
